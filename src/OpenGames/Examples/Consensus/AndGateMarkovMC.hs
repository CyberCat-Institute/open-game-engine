...
import           Debug.Trace
import           OpenGames.Engine.OpenGames hiding (fromLens,Agent,fromFunctions,discount,nature)
...
import           Numeric.Probability.Distribution hiding (map, lift, filter, uniform)
...
andGateTestPrior = do
  deposits <- uniform [replicate 3 x | x <- [0.0, 1.0 .. 10.0]]
...
--                      -> OpenGame
--                           (MonadOptic (Msg Double))
--                           (MonadContext (Msg Double))
...
--                              Kleisli CondensedTableV (([Double], Bool), [Double], Double) Bool,
--                              Kleisli CondensedTableV (([Double], Bool), [Double], Double) Bool])
--                           ('[RIO (GLogFunc (Rdr Double)) (DiagnosticsMC Double),
--                              RIO (GLogFunc (Rdr Double)) (DiagnosticsMC Double),
--                              RIO (GLogFunc (Rdr Double)) (DiagnosticsMC Double),
--                              RIO (GLogFunc (Rdr Double)) (DiagnosticsMC Double),
--                              RIO (GLogFunc String) (DiagnosticsMC Bool),
--                              RIO (GLogFunc String) (DiagnosticsMC Bool),
--                              RIO (GLogFunc String) (DiagnosticsMC Bool)])
--                           ([Double], Bool)
--                           ()
...
extractContinuation :: (MonadOptic String) s () s () -> s -> StateT Vector IO ()
extractContinuation (KleisliOptic v u) x = do
  (z,a) <- ST.lift (v x)
  u z ()

-- extract next state (action)
extractNextState :: (MonadOptic String) s () s () -> s -> IO s
extractNextState (KleisliOptic v _) x = do
  (z,a) <- v x
  pure a

extractContinuation2 :: (MonadOptic String) Double () ([Double],Bool) () -> Double -> StateT Vector IO ()
extractContinuation2 (KleisliOptic v u) x = do
  (z,a) <- ST.lift (v x)
  u z ()

-- extract next state (action)
extractNextState2 :: (MonadOptic String) Double () ([Double],Bool) () -> Double -> IO ([Double],Bool)
extractNextState2 (KleisliOptic v _) x = do
  (z,a) <- v x
  pure a
...
contextCont parameters iterator strat initialAction = KleisliContext (pure ((),initialAction)) (\_ action -> determineContinuationPayoffs parameters iterator strat action)

contextCont' parameters iterator strat initialAction = KleisliContext (pure ((),initialAction)) (\_ action -> determineContinuationPayoffs' parameters iterator strat action)

contextCont3 iterator initialAction = KleisliContext (pure ((),initialAction)) (\_ action -> determineContinuationPayoffs3 iterator action)
