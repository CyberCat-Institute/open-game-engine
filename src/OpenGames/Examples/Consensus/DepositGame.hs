...
generateGame "depositStagePlayer" ["name", "minDeposit", "maxDeposit", "incrementDeposit"] $ GBlock ["costOfCapital"] []
  [line [param "costOfCapital"] []
          [|dependentDecision name (const [minDeposit, minDeposit + incrementDeposit .. maxDeposit])|]
        ["deposit"] [[|-deposit * costOfCapital|]]]
  ["deposit"] []

generateGame "playingStagePlayer" ["name", "moves", "payoutCondition"] $  GBlock ["observation", "bribe"] []
  [line (map param ["observation", "bribe"]) [] [|dependentDecision name (const moves)|] ["move"] [[|payoff + if bribePaid then bribe else 0|]]]
...
generateGame "fullThing" ["numPlayers", "reward", "costOfCapital", "maxBribe", "successfulAttackPayoff", "payoffParameter"] $
  [line [ [| replicate numPlayers costOfCapital |] ] ["discard1"] [| population [depositStagePlayer ("Player " ++ show n) 0 10 0.1 | n <- [1 .. numPlayers]] |] ["deposits"] [ [| replicate numPlayers () |] ],
   ...
   line [ [| replicate numPlayers (deposits, bribe) |] ] ["discard2"] [| population [playingStagePlayer ("Player " ++ show n) [True, False] (const False) | n <- [1 .. numPlayers]] |] ["moves"] [ [| zip (payoffInt payoffParameter reward deposits (obfuscate moves)) bribesAccepted |] ],
