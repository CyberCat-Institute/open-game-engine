...
langaugeKeywords =
  [ "if",
    ...
    "do",
    "let",
    "in", -- language keywords
    "inputs",
    "outputs", -- the covariant arguments
    "feedback",
    "returns", -- the contravariant arguments
...
parseLit =
  LString <$> Tok.stringLiteral lexer
...
doNotation =
  ...
  where
    ...
    statement =
      try ((Just <$> identifier <* reservedOp "<-") `pair` expr)
...
bracketed =
  ( do
      e1 <- expr -- first check we have at least one element
      do
        e2 <- comma *> expr -- then expect a comma
        ...
        ( comma *> (LList . (\x -> e1 : e2 : x) <$> commaSep expr)
          ...
          <|> pure (LList [e1, e2]) -- if there is no comma, check if there is `..` to parse a range
...
comprehension = do
  ...
  where
    ...
    parseStmt =
      LBindS <$> parsePattern <*> (reservedOp "<-" *> expr)
        <|> LLetS <$> (reserved "let" *> parsePattern) <*> expr
        <|> LNoBindS <$> expr

term :: Parser Lambda
term =
  parens (try parseTuple <|> expr)
    <|> ifExp
    <|> lambda
    <|> variable
    <|> number
...
parseTwoLines kw1 kw2 parseP parseE =
  pair
    (reserved kw1 *> colon *> commaSep parseP <* semi)
...
parseVerboseSyntax parseP parseE =
  do
    (input, feedback) <- try (parseInput parseP parseE <* parseDelimiter) <|> pure ([], [])
