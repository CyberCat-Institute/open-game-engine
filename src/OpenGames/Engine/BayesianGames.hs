...
deviationsInContext ::
  (Show x, Show y, Ord y, Show theta) =>
  Double ->
  Agent ->
  x ->
  theta ->
  ...
  (y -> Payoff) ->
  [y] ->
  [DiagnosticInfoBayesian x y]
deviationsInContext epsilon name x theta strategy u ys =
  [ DiagnosticInfoBayesian
      { equilibrium = strategicPayoff >= optimalPayoff - epsilon,
        player = name,
...
dependentDecision name ys =
  OpenGame
    { play = \(a :- Nil) ->
        let v x = do y <- runKleisli a x; return ((), y)
...
dependentEpsilonDecision epsilon name ys =
  OpenGame
    { play = \(a :- Nil) ->
        let v x = do y <- runKleisli a x; return ((), y)
            u () r = modify (adjustOrAdd (+ r) r name)
...
(+++) ::
  ...
  (Unappend a1, Unappend a2, RepNothing b1, RepNothing b2) =>
        ...
        case unappend as of
          ((a1, a2) :: (List a1, List a2)) ->
            let xs1 = [((z, x1), p) | ((z, Left x1), p) <- decons h]
             ...
             in -- Warning: evil laziness trick
