...
-- Defining the necessary types for outputting information of a game
data DiagnosticInfo f x y = DiagnosticInfo
  { equilibrium     :: Bool
  , player          :: String
  , optimalMove     :: y
  , strategy        :: f y
...
-- prepare string information for game
showDiagnosticInfo :: (Show y, Ord y, Show x, Show (f y)) => DiagnosticInfo f x y -> String
...
-- output string information for a subgame expressions containing information from several players
showDiagnosticInfoL :: (Show y, Ord y, Show x, Show (f y)) => [DiagnosticInfo f x y] -> String
showDiagnosticInfoL [] = "\n --No more information--"
...
checkEqL :: (Show y, Ord y, Show x, Show (f y)) => [DiagnosticInfo f x y] -> String
...
instance (Show y, Ord y, Show x, Show (f y)) => Apply ShowDiagnosticOutput [DiagnosticInfo f x y] String where
  apply _ x = showDiagnosticInfoL x


data PrintIsEq = PrintIsEq

instance (Show y, Ord y, Show x, Show (f y)) => Apply PrintIsEq [DiagnosticInfo f x y] String where
  apply _ x = checkEqL x


data PrintOutput = PrintOutput

instance (Show y, Ord y, Show x, Show (f y)) => Apply PrintOutput [DiagnosticInfo f x y] String where
  apply _ x = showDiagnosticInfoL x
