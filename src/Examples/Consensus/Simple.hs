...
andGateGame_src = Block ["aliceStake", "bobStake", "(bribe :: Double)"] []
  [Line ["\"Alice\"", "()"] [] "roleDecision [False, True]" ["aliceDecision"] ["alicePayoff"],
   ...
   Line ["aliceStake", "bobStake", "carol (aliceDecision && bobDecision)"] ["()"] "fromFunctions (reward penalty) id" ["alicePayoff", "bobPayoff"] ["()"]]
  ["bobDecision"] []
...
consensusGame_src = Block ["bribe"] []
  [Line ["\"Alice\"", "()"] [] "roleDecision [0.0 .. 10.0]" ["aliceStake"] ["-costOfCapital*aliceStake"],
   ...
   Line ["aliceStake", "bobStake", "bribe"] [] "andGateGame penalty" ["bobDecision"] []]
  ["bobDecision"] []
...
attackerGame_src = Block [] []
  [Line [] [] "nature (prior bribeBudget)" ["bribe"] [],
   Line ["bribe"] [] "consensusGame penalty" ["bobDecision"] []]
  [] []
