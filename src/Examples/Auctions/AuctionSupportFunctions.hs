...
lotteryPayment resPrice kmax noLottery counterWinner lotteriesGiven ((name, bid, winner) : ls) =
  if winner
    then
      if counterWinner < noLottery
        then (name, resPrice) : lotteryPayment resPrice kmax noLottery counterWinner lotteriesGiven ls
        else (name, kmax) : lotteryPayment resPrice kmax noLottery counterWinner lotteriesGiven ls
    else
      if noLottery > lotteriesGiven
        then (name, resPrice) : lotteryPayment resPrice kmax noLottery (counterWinner + 1) (lotteriesGiven + 1) ls
...
lotteryPayment2 resPrice kmax noLottery counterWinner lotteriesGiven ((name, bid, winner) : ls) =
  if winner
    then
      if counterWinner < noLottery
        then (name, resPrice) : lotteryPayment2 resPrice kmax noLottery counterWinner lotteriesGiven ls
        else (name, pay) : lotteryPayment2 resPrice kmax noLottery counterWinner lotteriesGiven ls
    else
      if noLottery > lotteriesGiven
        then (name, resPrice) : lotteryPayment2 resPrice kmax noLottery (counterWinner + 1) (lotteriesGiven + 1) ls
        else (name, 0) : lotteryPayment2 resPrice kmax noLottery (counterWinner + 1) lotteriesGiven ls
  where
    pay = kmax - (fromIntegral noLottery) / 2 * (kmax - resPrice)
...
auctionPayment paymentFunction reservePrice kPrice winnerSlots noLotteries ls =
  ...
  where
    kMax = kMaxBid kPrice ls
    kThreshold = kMaxBid winnerSlots ls

auctionPaymentResPrice ::
  (Ord v) =>
  -- | Payment function
  (v -> v -> Int -> Int -> Int -> [(n, v, Bool)] -> [(n, v)]) ->
  Int ->
...
auctionPaymentResPrice paymentFunction kPrice winnerSlots noLotteries (ls, reservePrice) =
  ...
  where
    kMax = kMaxBidReservePrice reservePrice kPrice ls
