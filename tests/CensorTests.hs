...
-- Deposit Game
-------------------------------------------------------------------------------------------------
generateGame "depositGameTH" [] $ Block ["costOfCapital"] []
  [mkLine [] [] [|dependentDecision "Dave"  (const [0.0 .. 10.0])|] ["daveStake"]  [[|-costOfCapital*daveStake|]],
   mkLine [] [] [|dependentDecision "Erika" (const [0.0 .. 10.0])|] ["erikaStake"] [[|-costOfCapital*erikaStake|]],
   mkLine [] [] [|dependentDecision "Frank" (const [0.0 .. 10.0])|] ["frankStake"] [[|-costOfCapital*frankStake|]]]
...
-- Censor Game
-------------------------------------------------------------------------------------------------
generateGame "censorGameTH" ["censorPenalty"] $ Block ["daveStake", "erikaStake", "frankStake", "bribe"] []
  [mkLine (map param ["daveStake", "bribe"]) [] [|dependentDecision "Dave" (const [Honest, Censor])|] ["daveMove"] [[|case daveMove of {Honest -> davePayoff; Censor -> davePayoff + bribe}|]],
   mkLine [param "erikaStake"] [] [|dependentDecision "Erika" (const [Honest, Censor])|] ["erikaMove"] [param "erikaPayoff"],
   mkLine [param "frankStake"] [] [|dependentDecision "Frank" (const [Honest, Censor])|] ["frankMove"] [param "frankPayoff"],
   mkLine [[|(daveStake, erikaStake, frankStake)|], [|carolObservation (daveMove, erikaMove, frankMove)|]] [] [|fromFunctions (uncurry (payoffs censorPenalty)) id|] ["davePayoff", "erikaPayoff", "frankPayoff"] []]
  [[|daveMove|]] []
...
generateGame "fullCensorGameTH" ["censorPenalty"] $ Block [] []
  [mkLine [[|0.05|]] [] [|depositGameTH|] ["daveStake", "erikaStake", "frankStake"] [],
   mkLine (map param ["daveStake", "erikaStake", "frankStake"] ++ [[|0|]]) [] [|censorGameTH censorPenalty|] ["daveMove"] []]
  [] []
...
generateGame "bribeGameTH" [] $ Block ["target", "move", "bribe"] []
  [mkLine [param "target", [|()|]] [] [|roleDecision [()]|] ["dummy"] [[|case move of {Honest -> 0; Censor -> bribe}|]]]
  [] []
...
generateGame "censorBribeGameTH" ["censorPenalty", "bribeBudget"]
  [mkLine [[|0.05|]] [] [|depositGame|] ["daveStake", "erikaStake", "frankStake"] [],
   mkLine [] [] [|nature (uniform [0, bribeBudget])|] ["bribe"] [],
   mkLine (map param ["daveStake", "erikaStake", "frankStake", "bribe"]) [] [|censorGame censorPenalty|] ["daveMove"] []]
